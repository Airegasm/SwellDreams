...
function ensureCharInflateFlowAssignments() {
  ...
  if (settings.activePersonaId) {
    ...
    if (activePersona) {
      ...
      // Sync buttons (pass the in-memory persona to avoid stale re-read from disk)
      // syncPersonaAutoGeneratedButtons will save if buttons changed
      const buttonsChanged = syncPersonaAutoGeneratedButtons(activePersona.id, activePersona.assignedFlows, activePersona);

      // If flows changed but buttons didn't (so sync didn't save), save now
      if (personaFlowsChanged && !buttonsChanged) {
        savePersonaAsync(activePersona).catch(err => console.error('Failed to save persona flows:', err));
      }
    ...
    if (updatedCharacter.charInflateAutoLoadControls && updatedCharacter.isPumpable) {
      ...
      if (ciSettings.activePersonaId) {
        ...
        if (ciPersona) {
          ...
          // Pass in-memory persona to avoid stale re-read
          const ciBtnsChanged = syncPersonaAutoGeneratedButtons(ciPersona.id, ciPersona.assignedFlows, ciPersona);
          if (ciFlowsChanged && !ciBtnsChanged) {
            savePersonaAsync(ciPersona).catch(err => console.error('Failed to save persona flows:', err));
          }
          if (!sessionState.flowAssignments.personas) sessionState.flowAssignments.personas = {};

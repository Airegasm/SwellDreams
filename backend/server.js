 ...
 */
function extractButtonPressLabels(flow) {
  if (!flow || !flow.nodes) return [];
  return flow.nodes
    .filter(n => n.type === 'button_press' && n.data?.label)
...
/**
 * Sync buttons for all characters that have a specific flow assigned
...
function syncButtonsForFlowChange(flowId) {
  ...
  for (const character of characters) {
    ...
    if (assignedFlows.includes(flowId)) {
      const updated = syncAutoGeneratedButtons(character.id, assignedFlows);
